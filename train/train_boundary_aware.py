...
if __name__ == '__main__':
    ...
    if pretrain:
        ...
        unet = torch.load(pretrain_path)
        siamese_net_state_dict = model.state_dict()
        pretrained_keys = set(unet.keys())
        siamese_keys = set(siamese_net_state_dict.keys())

        missing_keys = siamese_keys - pretrained_keys
        unexpected_keys = pretrained_keys - siamese_keys
    ...
    train_dataloader, valid_dataloader = get_siamese_train_loaders(train_file,valid_file, data_directory,batch_size, with_boundary=True)
